...
from utils.optimtools import single_batch_group_probs, get_d_moomtl
...
def train_model(item_groups, user_neighbors):
    ...
    print('----- Running single objective optimization with recloss constraint -----')
    for epoch in range(1, args.max_epoch+1):
        ...
        for _, (users, pos_items) in enumerate(train_loader):
            ...
            for i in range(2):
                optimizer.zero_grad()
                if i==0:
                    rsd.backward(retain_graph=True)
                else:
            ...
            if recloss.detach().cpu().item() <= args.thre:
                loss_total = rsd
            else:
                grads = torch.stack([torch.cat(grads[i]) for i in range(2)])
                weight_vec = get_d_moomtl(grads, recloss, args.thre)
                normalize_coeff = n_tasks / torch.sum(torch.abs(weight_vec))
                weight_vec = weight_vec * normalize_coeff
                loss_total = weight_vec[0] * rsd + weight_vec[1] * recloss
        ...
        if args.mode in {'sp','eo'}:
            value = rsd
        elif args.mode in {'nsp','neo'}:
            value = rsdn
        if earlystopper(value, model) is True:
            break
...
if __name__ == "__main__":
    ...
    assert args.mode in {'sp','eo','nsp','neo'}
