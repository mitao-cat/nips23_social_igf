...
def train_model(item_groups, user_neighbors):
    ...
    print('----- Running post reconstruction baseline. -----')
    for epoch in range(1, args.max_epoch+1):
        ...
        total_reconstruct_loss, total_recreg_loss, total_reg1_loss, total_reg2_loss, total_bprloss = 0, 0, 0, 0, 0
        ...
        for _, (users, pos_items) in enumerate(train_loader):
            ...
            bprloss, regloss = model.calculate_loss(pos_scores, neg_scores) # bprloss not used for training!
            pos_labels, neg_labels = teacher.calculate_pos_neg_scores(users, pos_items, neg_items)
            reconstruct_loss = (torch.mean(torch.square(pos_scores-pos_labels)) + torch.mean(torch.square(neg_scores-neg_labels))) / 2
            ...
            if mode in {'sp','eo','nsp','neo'}:
                ...
                loss = reconstruct_loss + args.reg*regloss + args.reg1*rsd
                ...
                loss = reconstruct_loss + args.reg*regloss + args.reg1*rsd_sp + args.reg2*rsd_nsp
            loss.backward()
            optimizer.step()
            total_bprloss += bprloss.detach().cpu().item()
            total_recreg_loss += args.reg*regloss.detach().cpu().item()
            total_reconstruct_loss += reconstruct_loss.detach().cpu().item()
            total_neg_sample_time += (sample_end_time - sample_start_time)
            total_train_time += (time.time() - sample_end_time)
        
        nb = len(train_loader)
        avgreconstruct, avgrecreg, avgreg1, avgreg2, avgbpr = \
            total_reconstruct_loss/nb, total_recreg_loss/nb, total_reg1_loss/nb, total_reg2_loss/nb, total_bprloss/nb
        avgloss = avgreconstruct + avgrecreg + avgreg1 + avgreg2
        print('Time:{:.2f}s = sample_{:.2f}s + train_{:.2f}s\tAvgLoss:{:.4f} = reconstruct_{:.4f} + reg_{:.4f} + {}_{:.4f} + {}_{:.4f} (bpr={:.4f})'.format(
            time.time()-epoch_start_time, total_neg_sample_time, total_train_time, 
            avgloss, avgreconstruct, avgrecreg, args.mode, avgreg1, 'n'+args.mode, avgreg2, avgbpr))
        model.eval()
        Rec = model.predict().detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    args.device, args.reg, args.max_epoch = device, 0, 200  # model_regularization=0!
    ...
    model, teacher = BaseMF(args).to(device), BaseMF(args).to(device)
    teacher.load_state_dict(torch.load('./data/{}/best_model.pth.tar'.format(args.dataset)))
